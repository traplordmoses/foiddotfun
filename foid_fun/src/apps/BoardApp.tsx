// src/apps/BoardApp.tsx
// MIFOID_LOREBOARD.APP — the board, extracted from /board so ONE
// implementation renders in BOTH presentations (multi-window plan §4):
//   - the /board route (BoardAppCore presentation="route": today's page
//     verbatim — mobile tree + desktop main + particles + titlebar)
//   - a desktop-shell window (default export: presentation="window" wrapped
//     in the route's ErrorBoundary + Suspense; OSWindow provides the frame
//     and titlebar, this renders only the window BODY)
//
// Stage-B blockers handled here (plan §4 BOARD row):
//   (a) usePanZoom's window-level keyboard (space-pan, +/=/−/0) takes a
//       keyboardEnabled option gated on useWindowActivity().focused, as do
//       the page-level P-to-propose, pending-item arrow keys, and Konami
//       listeners. Route presentation is always "focused", so standalone
//       behavior is unchanged. (Wheel/pointer gestures were element-scoped
//       already.)
//   (b) .board-particles is position:fixed page furniture (dressing the
//       wallpaper AROUND the route's window). In the shell the canvas fills
//       the window edge-to-edge and is opaque, so scoped particles would be
//       invisible paint cost — the window presentation drops them and the
//       desktop wallpaper provides the ambience. Route keeps them.
//   (c) useWindowActivity().minimized pauses background work while parked
//       in the dock: useBoardData's scheduled refetch ticks no-op (block
//       watcher stays subscribed, cheap; catch-up fires on restore) and the
//       presence cursor is cleared once so other clients don't see a frozen
//       ghost for 10s. The presence CHANNEL stays subscribed — restore is
//       instant and inbound peer updates are cheap.
//   (d) useViewportZoomLock takes the canvas ref in the shell so its
//       pinch/Ctrl+wheel preventDefault only fires for events originating
//       inside the board canvas — never over other windows/wallpaper.
//       Route keeps the historical page-wide lock.
//   (e) Drag-drop proposing was already element-scoped to the canvas div
//       (onDragOver/Drop on containerRef) — nothing page-level to unwind.
//   - Viewport takeovers (PlacementModal, PaintEditor, OnboardingTour, the
//     flag confirm inside PlacementCard) render through <BodyPortal>:
//     .vista-window's backdrop-filter is a containing block that would trap
//     their position:fixed inside the frame. PaintEditor is a deliberate
//     body-level takeover in BOTH presentations — paint mode owns the
//     screen. BatchReviewModal already portals via the shared <Modal>.
//
// board.css is imported HERE — the shared location both the route and the
// shell load (it used to sit in board/page.tsx).
"use client";
...
import { useWindowActivity } from "@/hooks/os/useWindowActivity";
...
// ============================================================================
// MAIN BOARD COMPONENT — shared by /board (route) and BOARD.EXE (shell)
// ============================================================================

export type BoardPresentation = "route" | "window";

export function BoardAppCore({
  presentation = "route",
}: {
  presentation?: BoardPresentation;
}) {
  const isWindow = presentation === "window";
  // Shell activity contract: focused gates window-level keyboard listeners,
  // minimized pauses background work. Route presentation: always
  // { focused: true, minimized: false } — zero behavior change.
  const { focused: shellFocused, minimized: shellMinimized } =
    useWindowActivity();
  ...
  // keyboardEnabled: in the shell, space-pan and +/=/−/0 detach while
  // another window is foreground (blocker (a)); on the route it's always
  // true.
  const {
    scale,
    pan,
    ...
    bindStage,
    subscribeViewport,
  } = usePanZoom(containerRef, { keyboardEnabled: shellFocused });
  ...
  // Shell blocker (c), presence half: a minimized window can't emit
  // pointermoves (pointer-events off), so broadcasting stops naturally —
  // but without this, remote boards would show our frozen ghost until the
  // 10s stale sweep. Clear the cursor once on minimize instead. The
  // channel itself stays subscribed (keep-alive) so restore is instant.
  useEffect(() => {
    if (shellMinimized) sendPresenceCursor(null);
  }, [shellMinimized, sendPresenceCursor]);

  // Konami code (↑↑↓↓←→←→BA) → retro palette for 30s. Body class carries
  // the override; tokens.css rewrites --foid-* under .retro-mode.
  // Shell: only the foreground window listens (arrow keys are also the
  // pending-item nudge keys — see blocker (a)).
  useEffect(() => {
  ...
  // Browser zoom lock (pinch/Ctrl+wheel/double-tap). Route: page-wide, the
  // historical behavior. Shell (blocker (d)): scoped to the canvas element
  // so pinching over another window or the wallpaper is untouched.
  useViewportZoomLock(isWindow ? containerRef : undefined);
  ...
  // the window is minimized; a catch-up fetch fires on restore. Explicit
  // refetches (post-submit) are never gated.
  const {
    proposals,
  ...
  // (The old "VOTING (n)" dock count + flyout are gone — removal voting
  // lives in the placement lightbox now. PlacementModal reads the active
  // vote for its one placement on open; no standing multicall needed.)
  ...
  // Arrow keys — nudge the active pending item. Shell: foreground-only,
  // so arrowing through text in another window never moves a placement.
  useEffect(() => {
    if (!shellFocused) return;
    const onKey = (e: KeyboardEvent) => {
  ...
  }, [activeId, pending, clampToCanvas, setRect, shellFocused]);
...
`
  ...
  // flow that doesn't require drag-and-drop. Shell: foreground-only, so P
  // pressed over another window never pops the board's file picker.
  useEffect(() => {
    if (!shellFocused) return;
    const onKey = (e: KeyboardEvent) => {
    ...
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [busy, submittingProposals, showReviewModal, desktopPaintFile, announceSr, shellFocused]);
  ...
  // The window BODY — everything below the titlebar. Shared VERBATIM by the
  // route's .vista-window and the shell's OSWindow frame (both are flex
  // columns; .board-body flexes to fill). Full-bleed: the canvas IS the
  // window body. No right column: actions float on the canvas (BoardActions,
            ...
            <div className="vista-window__body vista-window__body--flush pray-panel__body board-body">
              ...
              <div className="board-canvas-wrap flex-1 min-h-0">
                  ...
                  >
                    ...
                    {/* Floating action affordance — bottom-center, screen
                         space. Idle: quiet "+ propose" chip (drag-and-drop
                         is the loud invitation). Pending: prominent
                         SUBMIT PROPOSAL (n) pill. Removal voting lives in
                         the placement lightbox (PlacementModal). */}
                    <BoardActions
                      submissionFeeWei={SUBMISSION_FEE_WEI}
  ...
  // Desktop viewport takeovers, shared by both presentations. On the route
  // they render inline inside <main> (today's DOM — position:fixed against
  // the viewport, and the mobile tree keeps its own PlacementModal
  // instance). In the shell they go through <BodyPortal>: the OSWindow
  // frame's backdrop-filter is a containing block that would otherwise trap
  // and clip them. PaintEditor as a body-level takeover is deliberate UX in
  // both presentations — paint mode owns the screen. BatchReviewModal
  // already self-portals via the shared <Modal>.
  ...
  // ── Shell window presentation ────────────────────────────────────────
  // OSWindow provides the .vista-window frame + titlebar; we contribute
  // the body and the portaled takeovers. No .board-particles (blocker (b):
  // the opaque canvas fills the window edge-to-edge, so scoped particles
  // would be invisible animation cost — the desktop wallpaper is the
  // ambience) and no mobile tree (the shell is lg:-and-up only).
  if (isWindow) {
    return (
      <>
        {boardBody}
        <BodyPortal>{desktopOverlays}</BodyPortal>
      </>
    );
  }

  // ── Standalone route presentation (today's page, verbatim) ──────────
  const mainView = (
 ...
 *  presentation wrapped in the same crash protection the /board route
 *  provides — a board failure downs one window, not the whole desktop.
 *  (BoardAppCore reads useSearchParams for ?debug/?celebrate, hence the
 *  Suspense; ssr:false in the shell means it never suspends on the server.) */
export default function BoardApp() {
  return (
    ...
    >
      ...
      >
        <BoardAppCore presentation="window" />
      </Suspense>
    </ErrorBoundary>
  );
}
