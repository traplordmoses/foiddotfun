...
export default function CompactMusicPlayer({ mountLogic = true }: CompactMusicPlayerProps) {
  ...
  return (
    <>
      ...
      {!isExpanded && (
        <div className="cmp-hover-zone" onMouseEnter={expand} onTouchStart={expand} />
      )}
      ...
      <style jsx global>{`
        ...
        /* --- Mobile: hide bar on small screens to avoid overlap with MobileNav --- */
        @media (max-width: 1023px) {
          :global(.cmp-hover-zone) { bottom: 56px; }
          :global(.cmp-bar) { bottom: 56px; border-radius: 12px 12px 0 0; }
        }
      `}</style>
    </>
  );
