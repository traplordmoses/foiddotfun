...
export default function PrayerJournalDrawer({
  ...
  // Focus lands here when the sheet opens (via <Sheet initialFocusRef>).
  const closeButtonRef = useRef<HTMLButtonElement | null>(null);

  // Esc to close — closes the popover first if one is open, then the drawer.
  // <Sheet> owns the listener; we just supply the layered behavior.
  ...
  return (
    <>
      <Sheet
        ...
        backdropClassName="journal-backdrop"
      ...
      <style jsx>{`
        ...
        :global(.journal-sheet) {
          /* Positioning, height, safe-area padding and overflow come
             from <Sheet/>; this class owns the drawer's material only. */
          z-index: 1000;
