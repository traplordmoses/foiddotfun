...
export default function PrayerReminderLink({ className = "" }: { className?: string }) {
  // Built after mount: the server has no local clock. Computing it during
  // render made the server's "#" and the client's data: URL disagree, and
  // React keeps the server's attribute on a hydration mismatch, so the link
  // never pointed at the calendar file.
  const [href, setHref] = useState("#");
  useEffect(() => {
    setHref(buildIcsHref());
  }, []);
