...
export async function GET() {
  if (!cached || Date.now() - cached.at > 30_000) {
    inflight ??= (async () => {
      ...
      const checks = { rpc: outcomes[0].status === "fulfilled" && outcomes[0].value, storage: outcomes[1].status === "fulfilled" && outcomes[1].value, prayerConfig: Boolean(process.env.OPENAI_API_KEY && (process.env.MOMMY_SESSION_SECRET || process.env.CRON_SECRET)) };
