...
export default function HomeClient({ crawler, initialNarrow }: HomeClientProps) {
  // Build-time constant: opted out (NEXT_PUBLIC_FOID_DESKTOP=0) → the
  // launcher window everywhere, exactly as pre-desktop production.
  if (!FOID_DESKTOP_ENABLED || crawler) return <HomeLauncher />;
  return <DesktopGate initialNarrow={initialNarrow} />;
}
...
function DesktopGate({ initialNarrow }: { initialNarrow: boolean }) {
