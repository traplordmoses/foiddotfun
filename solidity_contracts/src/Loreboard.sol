...
contract Loreboard {
    ...
    function propose(
        ...
        require(!_overlapsExistingPlacement(x, y, w, h), "Loreboard: overlaps existing placement");
    ...
    function finalize(uint256 proposalId) external {
        ...
        if (isApproved && _overlapsExistingPlacement(p.gridX, p.gridY, p.gridW, p.gridH)) {
            isApproved = false;
    ...
    /// @dev Check if a proposed rect overlaps any existing non-removed placement.
    function _overlapsExistingPlacement(int32 x, int32 y, uint32 w, uint32 h) internal view returns (bool) {
        for (uint256 i = 0; i < placementCount; i++) {
            Placement storage existing = _placements[i];
    ...
    /// @dev AABB collision check. Two rects overlap if they share area. Touching edges = no overlap.
    ///      Uses int64 to safely handle int32 + uint32 addition without overflow.
    function _rectsOverlap(
        int32 ax, int32 ay, uint32 aw, uint32 ah,
        int32 bx, int32 by, uint32 bw, uint32 bh
    ) internal pure returns (bool) {
        int64 aRight  = int64(ax) + int64(int32(aw));
        int64 aBottom = int64(ay) + int64(int32(ah));
        int64 bRight  = int64(bx) + int64(int32(bw));
        int64 bBottom = int64(by) + int64(int32(bh));

        return !(aRight <= int64(bx) || bRight <= int64(ax) ||
                 aBottom <= int64(by) || bBottom <= int64(ay));
    }
